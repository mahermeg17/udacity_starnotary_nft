...
it('lets user2 buy a star and decreases its balance in ether', async () => {
    ...
    assert.isAbove(Number(value), Number(starPrice), "can't estimate gas!");
});
...
it('can add the star name and star symbol properly', async () => {
    ...
    assert.equal(await instance.name.call(), 'My Start Notary');
    assert.equal(await instance.symbol.call(), 'MSTN');
});
...
it('lets 2 users exchange stars', async () => {
    ...
    let user2 = accounts[2];
    const starId1 = 6;
    await instance.createStar('star_1', starId1, { from: user1 });
    assert.equal(await instance.ownerOf(starId1, { from: user1 }), user1);

    const starId2 = 7;
    await instance.createStar('star_2', starId2, { from: user2 });
